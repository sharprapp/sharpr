...
export default function Logo({ size = 'md' }) {
  ...
  return (
    <div onClick={() => navigate('/')} style={{ display:'flex', alignItems:'center', gap:'10px', cursor:'pointer' }}>
      <div style={{ width:s, height:s, background:'#111827', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
        <svg width={Math.round(s*0.55)} height={Math.round(s*0.55)} viewBox="0 0 22 22" fill="none">
          <path d="M3 18 L11 4 L19 18" stroke="#4f8ef7" strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round" fill="none"/>
        </svg>
      </div>
